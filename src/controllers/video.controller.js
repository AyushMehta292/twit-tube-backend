...
const getUploadSignature = asyncHandler(async (req, res) => {
  ...
  if (!resourceType || !["video", "image"].includes(resourceType)) {
    throw new APIError(400, "resourceType must be video or image");
  }

  const params = getSignedUploadParams(resourceType);
  if (!params) {
  ...
  return res
    .status(200)
    .json(new APIResponse(200, params, "Upload signature generated"));
});
...
const publishAVideo = asyncHandler(async (req, res) => {
  ...
  if (!videoFile) throw new APIError(400, "Video File Must be Required");
  if (!thumbnail) throw new APIError(400, "Thumbnail File Must be Required");
  if (duration == null || duration === "") {
    throw new APIError(400, "Duration is required");
  }

  if (!isValidCloudinaryAssetUrl(videoFile, "video")) {
    throw new APIError(400, "Invalid video URL");
  }
  if (!isValidCloudinaryAssetUrl(thumbnail, "image")) {
    throw new APIError(400, "Invalid thumbnail URL");
  }
